	...
	\frametitle{ランダムなネットワークの作成}
		\begin{exampleblock}{アルゴリズム}
			\begin{enumerate}
				\item 初期構造の作成
					\begin{itemize}
						\item \alert{実空間}で8-Chain Model により初期構造を作成。
...
\begin{frame}
	\frametitle{「す抜け鎖」での一軸伸長}
		\begin{exampleblock}{一軸伸長：Z軸方向に二倍伸長}
			\begin{itemize}
				\item ストランド：す抜け鎖
				\item 四分岐ランダムネットワークモデル
		...
		\begin{block}{ゆらぎの入ったポテンシャル}
			\begin{itemize}
				...
				\item 系のポテンシャルエネルギーは、
					...
					\end{equation*}
					\normalsize
				\item これは、自然長で決まる定数項とゆらぎに起因した第二項とに分割でき、その和で以下となる。
					\footnotesize
					\begin{equation*}
