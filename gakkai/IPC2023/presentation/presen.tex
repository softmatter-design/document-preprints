...
\begin{frame}
    \frametitle{Adhesive Bonding Technology}
		...
		% 発表ノート
		\note{
			Let me start with this slide as background.
			\begin{enumerate}
				\item focusing on Energy conservation
				\begin{itemize}
					\item weight reduction of cars is a big issue.
					\item for that purpose, multi-materialization, using Aluminum, Magnesium, CFR(T)Ps is discussed.
					\item (POINT) these different colors
					\item in the process, adhesive bonding technology is a big key.
					\item these red lines are the adhesive are used for bonding.
				\end{itemize}
				\item requirements for network polymers used as adhesives are 
				\begin{itemize}
					\item not only high primary mechanical properties
					\item but also the ability to withstand fatigue tests
				\end{itemize}
			\end{enumerate}
		}
\end{frame}
	...
	\vspace{-1mm}
		...
		\note{
			\begin{itemize}
				% \item hyst
				% \begin{itemize}
					\item Mechanical Hysteresis is illustrated in this figure.
					\item \textcolor{red}{(POINT)}
					% \item Reduced stress comes out on unloading compared with loading one.
					\item this green area is equivalent to Energy dissipation during cycle
					\item Positive correlation with fracture energy had been reported by Payne.
				% \end{itemize}
				\item The origin of hysteresis loss is also categorized by Payne
				% \begin{itemize}
					\item We focuse on this Viscoelasticity based one
				% \end{itemize}
			\end{itemize}
		}
\end{frame}
...
% 			\begin{itemize}
% 				\item \textcolor{blue}{At room temp.}, SBR show rather lower strength
% 				\item \alert{Lower Temp. or Higher deformationrate improve strength}\footnote{
...
% 		\note{
% 			\begin{itemize}
% 				\item At room temperature, SBR:Stylene Butadinen Rubber show rather lower strength
% 				\item this blue one
% 				\item but lowering temperature the strength much improved.
% 				\item and also higher deformation rare.
% 			\end{itemize}
...
\begin{frame}
	...
	% \vspace{-2mm}
		...
		\note{
			\begin{itemize}
				\item On the fracture of rubber, 
				\item Andrews proposed a model focused on Stress fields around the Crack top area
				\item stress fields can be divided in two regions, stress loading zone and unloading one.
				\item \textcolor{red}{(POINT)}
				\item On the progress of the crack, 
					\begin{itemize}
						\item stress field is transit
						\item During this transition, Hysteresis Loss is occur and dissipate energy
						\item through this process, the progress of Crack is suppressed
					\end{itemize}
				\item So Bigger Hysteresis Loss results in  Higher Toughness.
			\end{itemize}
		}
\end{frame}
...
\begin{frame}
    \frametitle{Classical Theory of Rubber Elasticity}
		...
		\note{
			Here, the classical theories of rubber Elasticity are summerized.
			\begin{itemize}
				\item Considering junction points fluctuation, Phantom Network Model is proposed.
				\item in this model, Modulus is reduced depending on the functionality of junctions.
				\item \textcolor{red}{(POINT)}
				\item as this factor, one minus 2 over f: functionality
			\end{itemize}
		}
\end{frame}
		...
		\begin{block}{Effect of other strands (Combination of $G_c$ and $G_e$)}
			...
			\only<2>{
				\begin{columns}[T, onlytextwidth]
					...
					\vspace{2mm}
                    \centering
					\includegraphics[width=.8\textwidth]{Constrained_Juntion.pdf}
	
					\vspace{3mm}
					\includegraphics[width=.45\textwidth]{topological_effect_ring.png}
				\end{columns}
			}

			\note{
				\begin{itemize}
					\item In the vicinity of Junction Point
					\begin{itemize}
						\item Junction points are surrounded by \alert{adjacent strands.}
						\item So, Fluctuation of junctions are \alert{suppressed}. 
					\end{itemize}
					\item Effect is mainly two.
					\begin{itemize}
						\item One is Suppression of fluctuation of Junction Points
						\begin{itemize}
							\item Deviation from PNM and higher Gc
						\end{itemize}
						\item Other effect is that Strands Entangles each other
						\begin{itemize}
							\item Entanglements works as a Junction Point
							\item Generate additional $G_e$
						\end{itemize}
						\item So, Storage modulus $G$ is \alert{combination of $G_c$ and $G_e$}
					\end{itemize}
					\item \textcolor{red}{(CLICK)}
					\item Concerning these effect, two approaches were proposed
					\begin{itemize}
						\item One is Constrained Junction model, on the uniaxial deformation, constraints are released and $G$ approaches to $G_c$
						\item Other is topological effect, such as trapped entanglements 
						\item those were counted as Ge
					\end{itemize}
				\end{itemize}
			}
		\end{block}
...
\begin{frame}
	...
	\vspace{-2mm}
		...
		\note{
			\begin{itemize}
				\item These three models are the major recent approach for a evaluation of constraints.
				\item All recent models are based on Phantom Network Model.
				\item The latest Rubinstein's Slip-tube model" seems proper.
				\item \textcolor{red}{(Point and CLICK)}
				\item In that model, Gc and Ge is separated simply.
				\item Contribution of Ge will be reduced depending on deformation
			\end{itemize}
		}
\end{frame}
...
\begin{frame}
	\frametitle{Random Networks as a key for PNM}
		...
		\note{
			\begin{itemize}
				\item Introduction of Random Connectivity is known to a key for Phantom Network model.
				\item As a result, Criteria for PNM is fulfilled
				\item \textcolor{red}{(POINT)}
				\item these two previous works deal random network
				\item the networks treated in these work is not so simple, and discussion is complicated.
			\end{itemize}
		}
\end{frame}
...
\begin{frame}
	...
	\note{
		This is an objective of this work.
		\begin{itemize}
			\item Recent approach for rubber elasticity models are based on Phantom Network Model.
			\item Introducing random connectivity, MD simulation studies were carried out.
			\item To investigate the criteria for Phantom Network Model, Two model chains are used.
			\begin{enumerate}
				...
				\begin{itemize}
					\item for KG Chain:
					\item Excluded Volume Effect is exist
					\item and No mutual crossing of Strands
					\item Phantom chain has nothing, just connection.
				\end{itemize}
			\end{enumerate}
		\end{itemize}
	}
		...
		\begin{itemize}
			\item 8-Chain Model is used as starting structure in \alert{Real space}.
				\begin{itemize}
					...
					\item Topological model is generated. 
				\end{itemize}
			\item Randomness is introduced in \alert{topological space}, by \alert{edge exchange}, random connectivity is introduced for each node.	
			\item Corresponding real space structure is generated from this.
			\item According to e2e distance of strand, system size and multiplicity are varied to keep $\rho = 0.85$.
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item Conditions for Phantom and KG Chains are shown here.
			\item Important point is, 
			\item for phantom chain,
			\item Employing "Force Cap LJ" for 1, 3 interaction of inner chain segments,
			\item the angle is almost the same as KG chain 
			\item and e2e length of KG Chains is emulated in phantom chain.
			\item the conditions for KG Chain is very popular repulsive one.
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
    \frametitle{Reluxation of Initial Structure in KG Network}
		...
		\note{
			\begin{itemize}
				\item For KG chain NO mutual chain crossing will occur, 
				\item Relaxation of initial structure is important.
				\item we used this method: Slow Push Off
			\end{itemize}
		}
\end{frame}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item Keeping the density, Strand length were varied by shrinking the system.
			\item n=36 to 48
			\item Shorter chain length resulted in higher modulus.
			\item \textcolor{red}{(POINT left two)}
			\item WIth proper chain length, N=48, nice fit to PNM nature is found. 
			\item These results shows that 
			\item shoter chain reduced fluctuation mobility of junction point and resulted in deviation from PNM.
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item With proper chain length, N=48
			\item functionality effect on modulus were examined using f = 3, 4, 6 
			\item the density of starands are the same for all these three
			\item so the modulus is varied by functionality
			\item 1/3, half, 2/3
			\item and all fit to PNM.
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item based on the results of networks consist of phantom chain.
			\item KG Network was examined with same strand length(f=4 and N=48).
			\item For KG NW, Modulus is much higher than PNM Predict.
			\item \textcolor{red}{(POINT)}
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item To investigate the higher modulus for KG NW,
			\item Employing Z1-code, number of entanglements per chain Z was calculated.
			\item Z for NW is almost the same as homo-polymer melts.
			\item \textcolor{red}{(POINT left two)}
			\item 1.02 and 1.04
			\item almost the same number means proper relaxation performance.
			\item So this NW model is proper model
			\item additionally, this z=about one means almost one entanglement per chain
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
    \frametitle{Reduced Entanglements by NPT Model}
		...
		\note{
		\begin{itemize}
			\item to compare the fully relaxed and entangled model
			\item Employing NPT relaxation, less entangled model is generated
			\item POINT
			\item calculated $<Z>_{Z1}$ = 0.36
			\item Stress relaxation was compared
			\item equilibrated modulus for NPT one is much lower.
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item Adapting Slip-tube model estimation, 
			\item entanglement effect was confirmed
			\item Gc is the same
			\item depending on L, which is equal to Z
			\item Ge is different
			\item and total G is nicely fit to the measured ones
			\item So the measured modulus is fully explained.
			\item \textcolor{red}{(POINT left two)}
		\end{itemize}
	}
\end{frame}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			\item Let me move to the last item
			\item Using properly entangled model, the relaxation phenomena was checked
			\item Stress relaxation G(t) was measured by step Stretch
			\item G(t) is transformed to Dynamic Viscoelastic Spectrum
			\item $\tan \delta$ decayed on a time scale of this region
			\item \textcolor{red}{(POINT)}
...
\begin{frame}
	...
	\note{
		\begin{itemize}
			...
			\item NOW we are continuing our work for much longer entangled chain and opposite much shorter no entangled chain both.
		\end{itemize}
	}
\end{frame}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{itemize}
                    \item Adapting Slip-tube model estimation, trapped Entanglement effect was fully explained.
                    \item Hysteresis Loss was confirmed even around the long relaxation time zone.
                    \item This prolonged relaxation time can be attributed to the reduced mobility of the cross-linking points due to the
                    network structure
                \end{itemize}
			\end{itemize}
		\end{itemize}
...
% \begin{frame}
% 	\frametitle{ネットワーク構造でのZ1-cord}
% 		\begin{center}
% 			\includegraphics[width=.8\textwidth]{z1cord.png}
% 		\end{center}
% \end{frame}
