		...
		% \begin{block}{}
			\begin{columns}[T, onlytextwidth]
					...
					\begin{itemize}
						\item ヒステリシスロス
							\begin{itemize}
								...
								\item サイクル変形でエネルギー散逸
							\end{itemize}
						\item 破壊エネルギーと正の相関\footnote{
								\scriptsize{K.A.Grosch et al., Rub. Chem. Tech., 41, 1157(1968)}
							}
							\begin{itemize}
								\item 変形温度にも強く依存\\
								ガラス転移温度との距離？
							\end{itemize}
						\item 発生の起源と効果\footnote{
				...
				\begin{center}
					...
					\vspace{5mm}
					\includegraphics[width=\textwidth]{hyst_break.png}
				\end{center}
			\end{columns}
		% \end{block}
\end{frame}
			...
			\begin{columns}[T, onlytextwidth]
				...
				\begin{itemize}
					...
					\item Andrews 理論\footnote{
						\scriptsize
			{E. H. Andrews and Y. Fukahori, \\J. of Mat. Sci. 12, 1307 (1977)}
					}
						\begin{itemize}
							\item クラック近傍応力場
							\item クラック進展時に、
							\begin{itemize}
								\item \alert{ヒステリシス}由来
								\item {\color{red} エネルギー散逸}
							\end{itemize}
						\end{itemize}	
				\end{itemize}
				\column{.48\linewidth}
					\begin{center}
						\includegraphics[width=.55\textwidth]{crack.png}
					\end{center}
			\end{columns}
		\end{exampleblock}
		\vspace{-2mm}
...
% 					\item Because of surrounding strands, fluctuation of junctions are suppressed. 
% 				\end{itemize}
% 				% 架橋点はストランド経由で直接連結した架橋点以外の、
% 				% 	\alert{近接する多数のストランド（図中の×）に囲まれている。}
...
% 				\end{itemize}
% 				\column{.25\textwidth}
% 				% \includegraphics[width=\textwidth]{Constrained_Juntion_model.png}
	
% 				\includegraphics[width=\textwidth]{Constrained_Juntion.pdf}
% 			\end{columns}
% 		\end{block}
% 		\vspace{-2mm}
% 		\begin{block}{Topological relationships}
% 			\begin{columns}[totalwidth=1\textwidth]
% 				\vspace{-2mm}
% 				\column{.72\textwidth}
% 				\begin{itemize}
% 					\item ss\footnote{\scriptsize{P.J.Flory, J.Chem.Phys., 66, 12, 5720 (1977)}}
% 				\end{itemize}
% 				\column{.25\textwidth}
...
% 					\end{align*}
% 				\item \textcolor{blue}{``Affine NW Model''}\\[1mm]
% 					架橋点が巨視的変形と相似に移動。（Affine 変形）\footnote{
