...
%
\renewcommand{\baselinestretch}{0.83} %全体の行間調整
...
\subsection{力学応答の評価}
変形速度の異なるせん断変形（1e-3 $\sim$ 5e-5/$\tau$）時の SS カーブを、各種モデルの理論曲線と共に Fig. \ref{fig:deform} に示した。
変形速度の低減により、$\lambda<1$ 程度の小さなひずみでは PNM に漸近していた。

最長緩和時間の逆数程度のオーダーである変形速度（5e-4/$\tau$）で周期的なせん断変形（$\lambda = 1$）を付与した場合の力学的ヒステリシスを測定した結果を Fig. \ref{fig:hyst} に示した。
